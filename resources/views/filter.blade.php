...
<body>
    ...
    <main class="mt-3">
        <div class="mt-3 ms-5">
            <div><a class="AHover" href="{{route('home')}}">Home</a> <svg xmlns="http://www.w3.org/2000/svg" height="24px"
                    viewBox="0 -960 960 960" width="24px" fill="#777676">
                    <path d="M504-480 320-664l56-56 240 240-240 240-56-56 184-184Z" />
                </svg> <a class="AHover" href="{{ route('categoria', ['nombreCategoria' => $nombreCategoria])}}">{{ $nombreCategoria }}</a></div>
            <div class="d-flex mt-3">
                <div class="m-3 w-25">
                    ...
                    <form action="{{ route('categoria.filtrar', ['nombreCategoria' => $nombreCategoria]) }}"
                        method="get">
                        <div class="border-custom-2 mt-3">
                            ...
                            <div class="p-3">
                                ...
                                @foreach ($subcategories as $subcategory)
                                    <div class="m-2">
                                        <input class="checkbox" type="checkbox" name="subcategorias[]"
                                            value="{{ $subcategory->id }}"><label class="ms-3"
                                            for="subcategorias">{{ $subcategory->nombre }}</label>
                                    </div>
                                @endforeach
                            </div>
                            <div class="p-3">
                ...
                <div class="w-75">
                    @foreach ($products as $product)
                        <div class="container-custom m-3"><a class="d-flex"
                                href="{{route('producto', ['nombreCategoria' => $nombreCategoria, 'nombreProducto' => $product->nombre])}}"><img
